...
const Part6 = () => {
  ...
  const sendData = async (e) => {
    ...
    if (validate()) {
      ...
      const res = await fetch("https://fyp-react-app.herokuapp.com/register", {
        method: "Post",
        headers: {
      ...
      try {
        ...
        if (res.status === 201) {
          const resFile = await fetch(
            "https://fyp-react-app.herokuapp.com/upload",
            {
              method: "Post",
            }
          );
          const sendMail = await fetch(
            "https://fyp-react-app.herokuapp.com/sendEmail",
            {
              method: "Post",
              headers: {
                "Content-type": "application/json",
  ...
  return (
    <>
      <div id="part-6" className="p-5">
        ...
        <div className="mb-3">
          <label for="exampleFormControlInput0" className="form-label">
            Name
          </label>
          <input
            {...register("name", { required: true })}
            onChange={(e) =>
              dispatch({ type: "NAMESTATE", payload: e.target.value })
            }
            type="text"
        ...
        <div className="mb-3">
          ...
          <input
            onChange={(e) =>
              dispatch({ type: "EMAILSTATE", payload: e.target.value })
            }
            type="email"
            className="form-control rounded-pill"
            name="email"
        ...
        <div className="mb-3">
          ...
          <input
            onChange={(e) =>
              dispatch({ type: "PASSSTATE", payload: e.target.value })
            }
            type="password"
            className="form-control rounded-pill"
            name="pass"
        ...
        <div className="mb-3">
          ...
          <button
            ...
            type="button"
            className="btn btn-primary mt-3 ms-2 rounded-pill"
          >
            Sign Up
          </button>
        </div>
